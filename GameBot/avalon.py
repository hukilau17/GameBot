...
class Avalon(Game):
    ...
    async def av_info(self, message):
        ...
        if (await self.check_game(message)):
            ...
            info += '**Enabled settings:** %s\n' % ', '.join([FEATURE_NAMES[key] for key, value in self.features.items() if value])
            info += '**Disabled settings:** %s\n' % ', '.join([FEATURE_NAMES[key] for key, value in self.features.items() if not value])
            if self.merged:
    ...
    async def outcome(self, message, outcome):
        ...
        if (await self.check_running(message)):
            if message.channel.type != discord.ChannelType.private:
                await message.delete()
                await message.channel.send('Play your success or fail cards in a **private message** to %s, you doofus.' % self.bot.user.mention)
                return
